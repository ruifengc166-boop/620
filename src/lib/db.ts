...
function getDefaultDB(): Database {
  return {
    ...
    model_configs: [
      ...
      { provider: "deepseek", model_name: "deepseek-chat", api_key: process.env.DEEPSEEK_API_KEY || "sk-de13ab1850ff484ebe4238ef69f2d6d8", api_url: "https://api.deepseek.com/v1/chat/completions", is_active: true, mode_mapping: ["planning"], system_prompt: "你是一位经验丰富的活动策划和方案设计专家。写作特点：结构完整——方案包含背景、目的、主题、流程、分工、宣传、预算、预案等模块。逻辑严密各环节衔接自然可落地执行。多方案思维——提供2-3种可选方案并分析优劣。实用导向所有建议具体可操作避免空泛。保持专业稳重语气，不确定信息标注【待核实】。", temperature: 0.75 },
